export function generateReadme(packageJson: string) {
  return `# LLM Prompt: Generate GitHub-Style README with Inferred Features from package.json
...
3.  **Infer Features (New Instruction):** In the \`## ✨ Features\` section, analyze the key dependencies (\`dependencies\` and \`devDependencies\`). Based on the combination of technologies, **infer a list of *potential* technical features or characteristics.** Frame these points cautiously (e.g., "Likely includes...", "Potentially offers...", "Built with features like..."). **Clearly label these as inferred and add a note advising the user to review and update this section.** Examples of inferences:
...
4.  **Structure the Output:**
    ...
    * *(List other relevant scripts found in \`scripts\` with brief explanations)*
    ...
    * *(Add other significant categories inferred)*
